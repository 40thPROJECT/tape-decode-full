...
tape-decode-fast split capture.ldf pieces/ --parts 4

# 2. decode a piece on each machine, however you like
tape-decode-fast decode --profile NTSC_VHS --input-format flac \
    pieces/capture.part00.ldf --output out

# 3. bring the .tbc files back and join them, in tape order
tape-decode-fast merge pc1.tbc pc2.tbc pc3.tbc pc4.tbc \
...
```bash
tape-decode-fast insert --into tape.tbc --insert gap.tbc --dry-run
```
...
tape-decode-fast split captura.ldf piezas/ --parts 4
...
```bash
tape-decode-fast insert --into cinta.tbc --insert hueco.tbc --dry-run
```
